...
!     Store Mass matrices of local FDM solves
      real mr,ms,mt
      common /fastdm/ mr(lxx*2,levb),ms(lxx*2,levb),mt(lxx*2,levb)
...
!            call gen_fast_mass(df(1,e),sr(1,e),ss(1,e),st(1,e),
!     $                         mr(1,e),ms(1,e),mt(1,e),x,y,z)
...
c-----------------------------------------------------------------------
      ...
      subroutine gen_fast_mass(df,sr,ss,st,mr,ms,mt,x,y,z)
c
c     Generate fast diagonalization matrices for each element
c
      include 'SIZE'
...
c
      parameter(lxx=lx1*lx1)
      real df(lx1*ly1*lz1,1),sr(lxx*2,1),ss(lxx*2,1),st(lxx*2,1)
      real mr(lxx*2,1),ms(lxx*2,1),mt(lxx*2,1)
...
c
     ...
     $                      ,llr(e),lmr(e),lrr(e),zgm2(1,1),lx2,e)
           ...
           call set_up_fast_1D_sem_mass( sr(1,e),mr(1,e),lr,nr,lbr,rbr
     $                      ,llr(e),lmr(e),lrr(e),fldr(1,e),e)
         endif
         if (ifaxis) then
            xsum = vlsum(wxm2,lx2)
     ...
     $                      ,lls(e),lms(e),lrs(e),zgm2(1,2),ly2,e)
               ...
               call set_up_fast_1D_sem_mass( ss(1,e),ms(1,e),ls,ns 
     $               ,lbs,rbs,lls(e),lms(e),lrs(e),flds(1,e),e)
            endif
         endif
         if (if3d) then
            if (param(44).eq.1) then
     ...
     $                      ,llt(e),lmt(e),lrt(e),zgm2(1,3),lz2,e)
               ...
               call set_up_fast_1D_sem_mass( st(1,e),mt(1,e),lt,nt 
     $               ,lbt,rbt,llt(e),lmt(e),lrt(e),fldt(1,e),e)
            endif
         endif
c
c        Set up diagonal inverse
...
c-----------------------------------------------------------------------
      subroutine set_up_fast_1D_sem_mass(s,gm,lam,n,lbc,rbc,ll,lm,lr,
     $                                    rho,ie)

!     In this case I also return the local mass matrix
...
c
      ...
      real gm(lr2)            ! return mass matrix as well
...
!      call set_up_fast_1D_sem_op(g,bb0,bb1,l,r,ll,lm,lr,bh,jgl,1)
      ...
      call copy(gm,g,lx1*lx1)             ! Return mass matrix
      call transpose(gm(n*n+1),n,gm,n)    ! compute the transpose of gm
