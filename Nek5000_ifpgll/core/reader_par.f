...
C
      ...
      param(94) = 0    ! projection for helmholz solves (controled by ifprojfld) 
...
!-----------
      elseif (ifpgll) then
         if (lx1.eq.lx2) then
            if (nid.eq.0) write(6,45) lx1,lx2
   45    format('ERROR: lx1,lx2:',2i4,' must be different for IFPGLL')
            call exitt
         endif
!----------- 
      else
...
!     prabal. Need to check why this is not supported      
